...
for i in L:
	for j in L:
		if j != 0 and int(i) <= int(j):
			reduced_list.append(Fraction(i,j))
			if reduced_list[len(reduced_list)-1] == 1/2:     
				spot_on=True
...
if len(reduced_list) > 1:
	...
	a=lambda x: (x-Fraction(1,2))
	reduced_list.sort(key=a)

	temp_list=list(reduced_list)
	closest_1=min(reduced_list, key=lambda y: (abs(y-Fraction(1,2))))
	temp_list.remove(closest_1)
	temp=min(temp_list, key=lambda y: (abs(y-Fraction(1,2))))

	if (abs(closest_1-Fraction(1,2))) == (abs(temp-Fraction(1,2))):
		closest_2=temp
	for i in reduced_list:   
		fractions.append(str(i))
else:
	fractions.append(str(reduced_list[0]))
	if reduced_list[0] == Fraction(1,2):
		spot_on=True
	else:
		closest_1=reduced_list[0]
