...
def preferred_sequence(working_list = [], new_list = []):
    ...
    while True:
        ...
        if curr_loc == looking_for:
            ...
            working_list.pop()
            return preferred_sequence(working_list, new_list)
    #Replace pass above with your code (altogether, aim for around 24 lines of code)

def test_pq(expected_list, generated_list):
    working_list= PriorityQueue()
    for element in generated_list:
        working_list.insert(element)
    if working_list._data[1:len(working_list)+1] == expected_list:
        return 'Correct'
    else:
...
if len(pq._data[ : len(pq) + 1])>2:
    generated_list=list(reversed(preferred_sequence(pq._data[:len(pq) + 1])))
    print(generated_list)
else:
    print([])
print(test_pq(pq._data[1 : len(pq) + 1],generated_list))
