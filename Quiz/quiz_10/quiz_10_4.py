...
new_list=[]
def preferred_sequence(working_list = [], sorted_list = []):
    ...
    while True:
        ...
        if curr_loc == looking_for:
            ...
            for index in range(len(travelled_through) - 1, 0, -1):
                #print('Deleting ', working_list[travelled_through[index]])
                working_list[travelled_through[index]] = working_list[travelled_through[index - 1]]
            working_list.pop()
            preferred_sequence(working_list,sorted_list)
            return new_list
    #Replace pass above with your code (altogether, aim for around 24 lines of code)

def test_pq(expected_list, preferred_sequence, for_seed, length):
    generated_sequence= PriorityQueue()
    for element in generated_list:
        generated_sequence.insert(element)
    if generated_sequence._data[1:len(generated_sequence)+1] == expected_list:
        return True
    else:
        print(f'for_seed: {for_seed}\n length: {length}')
        print('Random List:',expected_list)
        print('preferred_sequence:',preferred_sequence)
        print('generated_sequence:',generated_sequence._data[1:len(generated_sequence)+1])
        input()
        return False
...
for for_seed in range(0, 101):
    ...
    for length in range(0, 101):
        ...
        sorted_l = sorted(pq._data[1 : len(pq) + 1], reverse=True)

        if len(pq._data[ : len(pq) + 1])> 1:
            generated_list=list(reversed(preferred_sequence(pq._data[:len(pq) + 1],sorted_list=sorted_l)))
            #print('Inside Main: ',generated_list)
        else:
            pass
            #print(pq._data[ : len(pq) + 1])
        test_pq(pq._data[1 : len(pq) + 1],generated_list, for_seed, length)
        del new_list[:]
        del generated_list[:]
        del pq
