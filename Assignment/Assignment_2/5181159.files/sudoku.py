...
'''
    ...
    def box_options_builder(self, x, y,function_name=0):
        for location in self.box_to_location[self.location_to_box[(x, y)]]:
            if self.sudoku[location[0]][location[1]] not in self.primary_set:
                ...
                if len(temp_list) == 1 and function_name!=4:
    ...
    def column_options_builder(self, y,function_name=0):
        for row in range(9):
    ...
    def row_options_builder(self, x,function_name=0):
        for column in range(9):
            if self.sudoku[x][column] not in self.primary_set:
                ...
                if len(temp_list) == 1 and function_name != 4:
                    ...
                    self.options_dict[(x, column)] = temp_list

    def build_all_options(self):
        for row in range(9):
            for column in range(9):
                self.box_options_builder(row, column,function_name=4)
                self.row_options_builder(row,function_name=4)
                self.column_options_builder(column,function_name=4)
    ...
    def forced_tex_output(self, function_name=0):
        ...
        if self.preassess(function_name=1):
            ...
            while flag == 1:
                ...
                for high_freq_nb in self.frequency:
                    # print(f'Trying {high_freq_nb[0]}')
                    for box in range(9):
                        in_box_flag = 0
                        empty_locations = []
                        for location in self.box_to_location[box]:
                            if self.sudoku[location[0]][location[1]] in self.primary_set:
                                if high_freq_nb[0] == self.sudoku[location[0]][location[1]]:
                                    # print(f'Rejecting {high_freq_nb[0]}')
                                    in_box_flag = 1
                                    break
                            else:
                        ...
                        if in_box_flag == 0:
                            ...
                            for row in range(9):
                                for column in range(9):
                                    if high_freq_nb[0] == self.sudoku[row][column]:
                                        loc_freq.append((row, column))
                            # remove columns
                            del_loc = []
                            for nb_loc in loc_freq:
                                for empty_loc in empty_locations:
                                    if nb_loc[1] == empty_loc[1] or nb_loc[0] == empty_loc[0]:
                                        del_loc.append(empty_loc)
                            for del_element in del_loc:
                                try:
                                    del empty_locations[empty_locations.index(del_element)]
                                except ValueError:
                                    pass

                            if len(empty_locations) == 1:
                                self.sudoku[empty_locations[0][0]][empty_locations[0][1]] = high_freq_nb[0]
                                try:
                                    del self.options_dict[(empty_locations[0][0],empty_locations[0][1])]
                                    print(f'{high_freq_nb[0]} deleted.')
