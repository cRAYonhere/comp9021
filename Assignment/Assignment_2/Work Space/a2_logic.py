...
'''
    ...
    def box_options_builder(self, x, y, ):
        for location in self.box_to_location[self.location_to_box[(x, y)]]:
            if self.sudoku[location[0]][location[1]] not in self.primary_set:
                ...
                if len(temp_list) == 1:
                    print(f'Only Option in Box: {temp_list[0]} Added at location ({location[0]},{location[1]})')
    ...
    def column_options_builder(self, y):
        for row in range(9):
            if self.sudoku[row][y] not in self.primary_set:
                ...
                if len(temp_list) == 1:
                    print(f'Only Option in Column: {temp_list[0]} Added  at location ({row},{y})')
    ...
    def row_options_builder(self, x):
        for column in range(9):
            if self.sudoku[x][column] not in self.primary_set:
                ...
                if len(temp_list) == 1:
                    print(f'Only Option in Row: {temp_list[0]} Added at location ({x},{column})')
    ...
    def forced_tex_output(self, function_name=0):
        if self.preassess(function_name=1):
            if function_name == 0 or function_name == 1:
                    ...
                    if self.frequency[element][1] > 1:
                        for box in range(9):
                            ...
                            for location in self.box_to_location[box]:
                                try:
                                    #if box == 3 and self.frequency[element][0] == 9:
                                        #print(location, self.options_dict[location])
                                    ...
                                    continue
                            if counter == 1:
                                print(f'Forced {self.frequency[element][0]} at location ({final_location[0]},{final_location[1]})')
    ...
    def update_sudoku(self):
        ...
        for key in self.options_dict:
            if len(self.options_dict[key]) == 1:
                print(f'Only Option for cell {self.options_dict[key][0]} at location ({key[0]},{key[1]})')
                ...
                for column in range(9):
                    ...
                    try:
                        if box_counter == 1 and self.options_dict[(box_x, box_y)]:
                            print(f'Singleton in Box: {column+1} at ({box_x},{box_y})')
                    ...
                    try:
                        if row_counter == 1 and self.options_dict[(row_x, row_y)]:
                            ...
                            print(f'Singleton in Row: {column+1} at ({row_x},{row_y})')
                    ...
                    try:
                        if column_counter == 1 and self.options_dict[(column_x, column_y)]:
                            ...
                            print(f'Singleton in Column: {column+1} at ({column_x},{column_y})')
...
sudoku=Sudoku('custom_sudoku_1.txt')
#sudoku.preassess()
